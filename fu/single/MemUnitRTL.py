...
class MemUnitRTL( Component ):
  ...
  def construct( s, DataType, PredicateType, CtrlType,
    ...
    MIN_ADDRESS   = AddrType( 0x0000 )
    MAX_ADDRESS   = AddrType( data_mem_size ) #TRIVIAL COULD CHANGE

    # Access bit
    s.accessBit = Wire( Bits1 )
    ...
    @s.update
    def comb_logic():
      s.accessBit = b1( 0 )  # Initialize to 0
      ...
      if s.recv_opt.msg.ctrl == OPT_LD:
        ...
        if AddrType( s.recv_in[in0].msg.payload ) < MIN_ADDRESS or AddrType( s.recv_in[in0].msg.payload ) > MAX_ADDRESS:
          # print(f"Error: Access to invalid memory address 0x{s.recv_in[in0].msg.payload:x}") // Unsupported
          # s.recv_in[in0].msg.predicate = b1( 0 ) // Doesn't work
          s.accessBit = b1( 1 ) # Set the access bit to 1 if access is invalid
        else:
          s.accessBit = b1( 0 ) # Set the access bit to 0 if access is valid
